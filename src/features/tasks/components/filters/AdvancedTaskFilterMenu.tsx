...
interface AdvancedTaskFilterMenuProps {
  visible: boolean;
...
export const AdvancedTaskFilterMenu: React.FC<AdvancedTaskFilterMenuProps> = ({
  visible,
  currentFilters,
  onFiltersChange,
  onClose,
  ...
  // Smart positioning: check if menu fits on the right, otherwise align to right edge or left of button
          ...
          >
            ...
            <View style={styles.section}>
              <View style={styles.sectionHeader}>
                <Text style={[styles.sectionTitle, { color: theme.textSecondary }]}>Статусы</Text>
                {tempFilters.statuses.length > 0 && (
                  <View style={[styles.badge, { backgroundColor: theme.primary }]}>
                    <Text style={styles.badgeText}>{tempFilters.statuses.length}</Text>
                  </View>
                )}
              </View>
              <View style={styles.checkboxList}>
                {STATUS_TABS.map((status) => {
                  const isSelected = tempFilters.statuses.includes(status.key as TaskStatus);
                  return (
                    <TouchableOpacity
                      key={status.key}
                      style={[
                        styles.checkboxItem,
                        {
                          backgroundColor: isSelected
                            ? status.color + '15'
                            : 'transparent',
                          borderColor: isSelected ? status.color : theme.border,
                        },
                      ]}
                      onPress={() => toggleStatus(status.key as TaskStatus)}
                    >
                      <View
                        style={[
                          styles.checkbox,
                      ...
                      <Text style={[styles.checkboxLabel, { color: theme.text }]}>{status.label}</Text>
                    </TouchableOpacity>
                  );
                })}
              </View>
